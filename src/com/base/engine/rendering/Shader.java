...
public class Shader{
	...
	// Used to get the modelviewprojectionMatrix in the postprocessing step
	private static Matrix4f PrePPMVP;
	...
	public void updateUniforms(Transform transform, Material material, RenderingEngine renderingEngine){
		...
		if(renderingEngine.getRenderPhase() == RenderPhase.AMBIENT){
			PrePPMVP = renderingEngine.getMainCamera().getViewProjection();
		}
		Matrix4f projectionMatrix = renderingEngine.getMainCamera().getProjection();
		Matrix4f viewMatrix = renderingEngine.getMainCamera().getViewMatrix();
		Matrix4f vpMatrix = renderingEngine.getMainCamera().getViewProjection();
		for(int i = 0; i < resource.getUniformNames().size(); i++){
			...
			if(uniformName.startsWith("R_")){
				...
				}else if(unprefixedUniformName.equals("biasMatrix")){
					setUniform(uniformName, renderingEngine.getBiasMatrix());
				}else if(unprefixedUniformName.equals("waterReflectionMatrix")){
					setUniform(uniformName, renderingEngine.getWaterReflectionMatrix().mul(worldMatrix));
				}else if(unprefixedUniformName.equals("clipPlane")){
					glUniform4f(resource.getUniforms().get(uniformName), renderingEngine.getClipPlaneEquation()[0], renderingEngine.getClipPlaneEquation()[1], renderingEngine.getClipPlaneEquation()[2], renderingEngine.getClipPlaneEquation()[3]);
				...
				}else if(uniformType.equals("sampler2DArray")){
					int samplerSlot = renderingEngine.getSamplerSlot(unprefixedUniformName);
					renderingEngine.getTextureArray(unprefixedUniformName).bind(samplerSlot);
					setUniformi(uniformName, samplerSlot);
				}else if(unprefixedUniformName.equals("kernel")){
					glUniform3(glGetUniformLocation(resource.getProgram(), "R_kernel"), renderingEngine.getKernel());
				}else if(uniformType.equals("vec3")) setUniform(uniformName, renderingEngine.getVector3f(unprefixedUniformName));
				else if(uniformType.equals("float")) setUniformf(uniformName, renderingEngine.getFloat(unprefixedUniformName));
				else if(uniformType.equals("DirectionalLight")) setUniformDirectionalLight(uniformName, (DirectionalLight) renderingEngine.getActiveLight());
				else if(uniformType.equals("PointLight")) setUniformPointLight(uniformName, (PointLight) renderingEngine.getActiveLight());
				else if(uniformType.equals("SpotLight")) setUniformSpotLight(uniformName, (SpotLight) renderingEngine.getActiveLight());
				...
				int samplerSlot = renderingEngine.getSamplerSlot(uniformName);
				material.getTextureArray(uniformName).bind(samplerSlot);
				setUniformi(uniformName, samplerSlot);
			}else if(uniformName.equals("animationJoints")){
				FloatBuffer jointsData = material.getFloatBuffer(uniformName);
				glUniformMatrix4(resource.getUniforms().get(uniformName), true, jointsData);
			}else if(uniformName.startsWith("T_")){
				if(uniformName.equals("T_MVP")) setUniform(uniformName, MVPMatrix);
				else if(uniformName.equals("T_model")) setUniform(uniformName, worldMatrix);
				else if(uniformName.equals("T_ViewProjection")) setUniform(uniformName, vpMatrix);
				else if(uniformName.equals("T_PrePPMVP")) setUniform(uniformName, PrePPMVP);
				else if(uniformName.equals("T_projection")) setUniform(uniformName, projectionMatrix);
				else if(uniformName.equals("T_view")) setUniform(uniformName, viewMatrix);
				...
				else if(uniformName.equals("C_forward"))setUniform(uniformName, renderingEngine.getMainCamera().getTransform().getTransformedRot().getForward());
				else if(uniformName.equals("C_up"))setUniform(uniformName, renderingEngine.getMainCamera().getTransform().getTransformedRot().getUp());
				else if(uniformName.equals("C_near"))setUniformf(uniformName, renderingEngine.getMainCamera().getzNear());
				else if(uniformName.equals("C_far"))setUniformf(uniformName, renderingEngine.getMainCamera().getzFar());
				else if(uniformName.equals("C_fov"))setUniformf(uniformName, renderingEngine.getMainCamera().getFov());
				else if(uniformName.equals("C_aspect"))setUniformf(uniformName, renderingEngine.getMainCamera().getAspect());
	...
	public void addAllUniforms(String shaderText){
		...
		while(uniformStartLocation != -1){
			...
			if(uniformName.endsWith("]")){
				int i = uniformName.lastIndexOf("[");
				uniformName = uniformName.substring(0, i);
			}
